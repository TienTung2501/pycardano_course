...
# Đơn giản hóa: Chỉ burn 1 NFT với thuộc tính (không cần random vì burn không tạo mới)
asset = {
    "name": "Pycardano_test_NFT_000",
}
...
# Kiểm tra UTxO tại main_address (nơi chứa NFT)
try:
    utxos = api.address_utxos(main_address)
    if not utxos:
        print("Main address does not have any UTXOs containing the NFT.")
        sys.exit(1)
except Exception as e:
    if e.status_code == 404:
        print("Receive address does not have any UTXOs. ")
        if network == "testnet":
            print(
...
if not exists(policy_skey_path) or not exists(policy_vkey_path):
    print(f"Không tìm thấy {policy_skey_path} hoặc {policy_vkey_path}. Cần tạo khóa khi phát hành NFT trước.")
    sys.exit(1)
...
# Không cần metadata cho burn (metadata chỉ dùng khi mint)

# Xử lý 1 NFT để burn (không cần loop)
