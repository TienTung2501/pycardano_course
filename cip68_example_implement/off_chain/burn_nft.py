...
def main():
    ...
    # Add issuer input for fees
    builder.add_input_address(issuer_addr)
    
    # If reference token found at script, need to unlock it
    # But for simple burn, we can just burn the user token
    # Reference token burn requires validator execution
    
    if not user_utxo:
        print("    [ERROR] User token not found!")
        return
    
    # Add user token UTxO as input
    builder.add_input(user_utxo)
    
    # Burn tokens (negative amount in mint)
    burn_assets = MultiAsset({
        policy_id: Asset({
            user_token_name: -1,  # Burn user token
        })
    })
    ...
    signed_tx = builder.build_and_sign(
        signing_keys=[issuer_skey, policy_skey, user_skey],
    ...
    try:
        ...
        print(f"    Note: Reference token (100) still at script - requires validator to burn")
